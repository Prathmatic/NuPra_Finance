...
-- 1. Tables (safe if they already exist)
...
create table if not exists public.couple_vaults (
...
create unique index if not exists one_vault_per_user on public.vault_members(user_id);
...
create unique index if not exists one_pending_invite_per_vault
  on public.vault_invites(vault_id) where accepted_at is null;
create index if not exists vault_invites_email_idx on public.vault_invites(invited_email);

create table if not exists public.vault_finance_state (
...
returns boolean language sql stable security definer set search_path = '' as $$
  select exists (
    select 1 from public.vault_members m
    where m.vault_id = p_vault_id and m.user_id = (select auth.uid()) and m.role = 'owner'
  );
$$;
...
-- 3. Core RPCs for Email Vault & Invite
...
begin
  ...
  if v_email is null or v_email = '' or v_email = lower(coalesce((select u.email from auth.users u where u.id = v_user_id), '')) then
  ...
  if exists (select 1 from public.vault_invites i where i.vault_id = p_vault_id and i.accepted_at is null) then
    select i.id into v_invite_id from public.vault_invites i where i.vault_id = p_vault_id and i.accepted_at is null for update;
...
returns table (
  ...
  where i.accepted_at is null
    and i.invited_email = lower(coalesce((select u.email from auth.users u where u.id = (select auth.uid())), ''));
$$;

drop function if exists public.accept_partner_invite(uuid);
create or replace function public.accept_partner_invite(p_invite_id uuid)
returns uuid language plpgsql security definer set search_path = '' as $$
declare
  v_user_id uuid := auth.uid();
  v_email text := lower(coalesce((select u.email from auth.users u where u.id = auth.uid()), ''));
  v_vault_id uuid;
begin
  if v_user_id is null or v_email = '' then raise exception 'Verified email login required'; end if;

  select i.vault_id into v_vault_id
  from public.vault_invites i
  join public.couple_vaults v on v.id = i.vault_id
  where i.id = p_invite_id and i.accepted_at is null and i.invited_email = v_email
  for update of i, v;
...
-- 4. Triggers (Drop if exists first to avoid duplicate trigger error)
...
drop trigger if exists enforce_two_vault_members on public.vault_members;
...
for select to authenticated using ((select private.can_view_profile(id)));
...
drop policy if exists couple_vaults_select_members on public.couple_vaults;
create policy couple_vaults_select_members on public.couple_vaults
for select to authenticated using ((select private.is_vault_member(id)));

drop policy if exists vault_members_select_members on public.vault_members;
...
drop policy if exists vault_finance_state_select_members on public.vault_finance_state;
create policy vault_finance_state_select_members on public.vault_finance_state
for select to authenticated using ((select private.is_vault_member(vault_id)));

drop policy if exists vault_finance_state_insert_members on public.vault_finance_state;
...
drop policy if exists vault_finance_state_update_members on public.vault_finance_state;
